/*
    useEffect : 컴포넌트가 렌더링될 때마다 특정 작업을 수행도록 설정한다.
    특정작업(side effect) : Time(setTimeout), Ajex
    useEffect : useEffect는 side effect라는 뜻이다.

    useEffect 4가지 종류:
    1. useEffect(callback) : 마운트 후, 리렌더링 될 때마다
    1. useEffect(callback, []) : 마운트 후
    1. useEffect(callback, [state]) : 마운트 후, state
    1. useEffect(callback, {return ();}, []) : 마운트 후, 언마운트 전
*/
